...
// InitializeGuildHandler initializes the message event listener for a specific guild
func (router *Router) InitializeGuildHandler(session *discordgo.Session, guildID string) {
	session.AddHandler(router.GuildHandler(guildID))
}
...
// GuildHandler provides the discordgo handler for the given router in the specified guild
func (router *Router) GuildHandler(guildID string) func(*discordgo.Session, *discordgo.MessageCreate) {
	return func(session *discordgo.Session, event *discordgo.MessageCreate) {
		...
		// Check for guild ID, handle messages only sent in the specified guild
		if message.GuildID != guildID {
			return
		}

		handleEventContent(router, content, session, event)
	}
}
...
func handleEventContent(router *Router, content string, session *discordgo.Session, event *discordgo.MessageCreate) {
